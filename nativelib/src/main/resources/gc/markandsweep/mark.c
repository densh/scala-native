...
word_t *inner_get_header(word_t *inner_ptr) {
    ...
    // Go up while the block is not a header. It uses the bitmap_copy, because
    // bitmap clears bits for marked blocks. No need to check for heap bounds
    // because heap_start is a header.
    ...
    if (header_unpack_tag(current) == tag_allocated &&
        bitmap_get_bit(heap->bitmap, current)) {
        return current;
    }
...
void scan_heap_after_overflow(Stack *stack) {
    ...
    while (block < heap->heap_end && !found) {
        word_t *chunk = (word_t *)((uintptr_t)block -
                                   (uintptr_t)block %
                                       (SMALLEST_CHUNK_SIZE * sizeof(word_t)));
        size_t block_size = header_unpack_block_size(chunk);
        word_t *chunk_end =
            chunk + SMALLEST_CHUNK_SIZE -
            (block_size > SMALLEST_CHUNK_SIZE / 2 ? 0 : block_size);
        while (block <= chunk_end && !found) {
            if (bitmap_get_bit(heap->bitmap_copy, block) &&
                !bitmap_get_bit(heap->bitmap, block) &&
                header_unpack_tag(block) == tag_allocated) {
                ...
                if (rtti.rt.id == __object_array_id) {
                    ...
                    for (int i = 0; i < size - 1; i++) {
                        ...
                        if (heap_in_heap(heap, field_addr) &&
                            bitmap_get_bit(heap->bitmap, field_addr)) {
                            assert(header_unpack_tag(field_addr) ==
                                   tag_allocated);
                    ...
                    while (ptr_map[i] != -1) {
                        ...
                        word_t *field =
                            (word_t *)(block[ptr_map[i] / sizeof(word_t) + 1]);
...
void mark(word_t *block) {
    ...
    if (((uintptr_t)block & (uintptr_t)0x7) == 0 &&
        bitmap_get_bit(heap->bitmap, block) && tag == tag_allocated) {
