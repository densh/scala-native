...
inline Heap *heap_alloc(size_t size) {
    Heap *heap = malloc(sizeof(Heap));
    size_t nb_words = size / sizeof(word_t);

    word_t *heap_start = mmap(NULL, MAX_SIZE, HEAP_MEM_PROT, HEAP_MEM_FLAGS,
...
inline int heap_in_heap(Heap *heap, word_t *block) {
    return block >= heap->heap_start && block < heap->heap_end;
}

inline int heap_cannot_be_const(Heap *heap, word_t *block) {
    return block >= heap->heap_start;
}

inline word_t *heap_next_block(Heap *heap, word_t *block) {
...
inline void heap_grow(Heap *heap, size_t nb_words) {
    nb_words = (nb_words + SMALLEST_CHUNK_SIZE - 1) / SMALLEST_CHUNK_SIZE *
               SMALLEST_CHUNK_SIZE;
...
inline void grow_heap(size_t nb_words) {
