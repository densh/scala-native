...
void chunk_allocator_add_chunk(ChunkAllocator *chunk_allocator, Chunk *block,
    ...
    while (remaining_size > 0) {
        ...
        assert(chunk_size >= SMALLEST_CHUNK_SIZE &&
...
Chunk *chunk_allocator_get_chunk(ChunkAllocator *chunk_allocator,
    ...
    while (list_index <= CHUNK_LIST_NUMBER - 1 &&
           (chunk = chunk_allocator->chunk_lists[list_index]->first) == NULL) {
        ++list_index;
    }
    if (chunk == NULL) {
        return NULL;
    ...
    if (received_chunk_size - SMALLEST_CHUNK_SIZE >= requested_chunk_size) {
        ...
        linked_list_remove_block(chunk_allocator->chunk_lists[list_index],
                                 chunk, requested_chunk_size);
        size_t remaining_chunk_size =
            received_chunk_size - requested_chunk_size;
        chunk_allocator_add_chunk(chunk_allocator, (Chunk *)remaining_chunk,
