...
void linked_list_print(LinkedList *list) {
    ...
    while (current != NULL) {
        ...
        printf("[%p (%zu | %zu)] -> ", current,
               header_unpack_block_size((word_t *)current),
               header_unpack_object_size((word_t *)current));
...
void linked_list_check(LinkedList *list, int index, Bitmap *bitmap) {
    ...
    while (current != NULL) {
        ...
        for (word_t *c = (word_t *)current + 1; c < (word_t *)current + size;
             c += 1) {
