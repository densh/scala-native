...
void free_list_print_stats(FreeList *list) {
    MemoryStats free_list_stats =
        free_list_get_stats(list->list, LINKED_LIST_NUMBER);
    printf("### STATS ###\n");
    free_list_print_block_counts(free_list_stats, LINKED_LIST_NUMBER);
    printf("total free: %zu / %zu\n", free_list_stats.total,
           list->size / sizeof(word_t));
    printf("max: %zu\n", free_list_stats.largest_word);
    printf("#############\n");
    free_list_print_chunks(list->chunk, LINKED_LIST_NUMBER);
    printf("#############\n");
    MemoryStats alloc_stats = free_list_get_stats(
        list->chunk_allocator->chunk_lists, CHUNK_LIST_NUMBER);
...
void check_chunks(FreeList *free_list) {
    for (int i = 0; i < LINKED_LIST_NUMBER; i++) {
        ...
        if (chunk != NULL) {
            word_t *chunk_end =
                chunk -
                (((uintptr_t)chunk % (SMALLEST_CHUNK_SIZE * sizeof(word_t))) /
                 sizeof(word_t)) +
                SMALLEST_CHUNK_SIZE;
            word_t *current = chunk;
            while (current != chunk_end) {
                if (bitmap_get_bit(free_list->bitmap, current)) {
                    assert(header_unpack_block_size(current) ==
...
void check_chunk_allocator(ChunkAllocator *chunk_allocator) {
    for (int i = 0; i < CHUNK_LIST_NUMBER; i++) {
        linked_list_check(chunk_allocator->chunk_lists[i],
                          i + LINKED_LIST_NUMBER, chunk_allocator->bitmap);
    }
}
