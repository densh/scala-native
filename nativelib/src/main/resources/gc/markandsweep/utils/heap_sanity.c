...
int check_is_header_in_linked_list(LinkedList *list, word_t *block) {
    ...
    while (current != NULL) {
        ...
        if (current_as_word_t == block)
            found = 1;
        assert(
            !(block > current_as_word_t && block < current_as_word_t + size));
...
void check_is_header_in_free_list(FreeList *list, word_t *block) {
        ...
        if (list->chunk[i] != NULL) {
            ...
            word_t *chunk_end =
                ...
                (((uintptr_t)chunk % (SMALLEST_CHUNK_SIZE * sizeof(word_t))) /
                 sizeof(word_t)) +
                SMALLEST_CHUNK_SIZE;
            int newFound = block + header_unpack_block_size(block) > chunk &&
                           block < chunk_end;
            assert(!newFound || !found);
            found |= newFound;
        }
    ...
    for (int i = 0; i < CHUNK_LIST_NUMBER; i++) {
        if (list->chunk_allocator->chunk_lists[i] != NULL) {
            int newFound = check_is_header_in_linked_list(
                list->chunk_allocator->chunk_lists[i], block);
            assert(!newFound || !found);
            found |= newFound;
        }
    }
...
void check_not_in_free_list(Heap *heap, word_t *block) {
    ...
    for (int i = 0; i < CHUNK_LIST_NUMBER; i++) {
        if (list->chunk_allocator->chunk_lists[i] != NULL) {
            check_not_in_linked_list(list->chunk_allocator->chunk_lists[i],
                                     block);
        }
    }
}
