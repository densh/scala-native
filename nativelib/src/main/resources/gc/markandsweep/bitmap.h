...
inline Bitmap *bitmap_alloc(size_t size, word_t *offset) {
    ...
    unsigned long nb_words =
        (size / sizeof(word_t) + BITS_PER_WORD - 1) / BITS_PER_WORD;
...
inline void bitmap_set_bit(Bitmap *bitmap, word_t *addr) {
    assert(addr >= bitmap->offset &&
           addr < bitmap->offset + bitmap->size / sizeof(word_t));
...
inline int bitmap_get_bit(Bitmap *bitmap, word_t *addr) {
    ...
    size_t inner_addr = addr - bitmap->offset;
    word_t bit = bitmap->words[WORD_OFFSET(inner_addr)] &
                 (1LLU << BIT_OFFSET(inner_addr));
    return bit != 0;
}
