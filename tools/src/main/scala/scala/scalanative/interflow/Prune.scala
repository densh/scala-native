...
object Prune {
  ...
  def deoptInsts(cfg: ControlFlow.Graph,
                 entry: Local,
  ...
  def deoptMethod(defn: Defn.Define,
                  cfg: ControlFlow.Graph,
                  entry: Local,
                  live: Seq[Val.Local]): Defn.Define = {
  ...
  def prune(defn: Defn.Define)(
      implicit linked: linker.Result): Seq[Defn.Define] = {
    ...
    def handleNext(next: Next): Next = next match {
      case next: Next.Label
          if next.weight == 0 && !cfg.find(next.name).inLoop =>
