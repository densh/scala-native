...
class Interflow(val orig: Map[Global, Defn.Define])(
    implicit val linked: linker.Result)
    extends Expand
    with Eval
    with Intrinsics
    with Log {
  val todo      = mutable.Queue.empty[Expanded]
  ...
  def apply(config: build.Config,
    ...
    linked.entries.foreach { entry =>
      linked.infos(entry) match {
        case info: Method =>
          interflow.expandRoot(entry)
        case _ =>
          ()
      }
    }
    interflow.expandLoop()
