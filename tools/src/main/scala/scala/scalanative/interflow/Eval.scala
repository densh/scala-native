        ...
        case Inst.Throw(v, unwind) =>
          ...
          unwind match {
            ...
            case Next.Unwind(name) =>
              return Inst.Jump(Next.Label(name, Seq(excv)))
            case _ =>
              unreachable
          }
      }
    ...
    op match {
          ...
          case _ =>
            ...
            val (msig, mtarget) = mmeth match {
              case Val.Global(name, _) =>
                expandNow(Expanded(name, margtys))
                  .map { defn =>
      ...
      case Op.Select(cond, thenv, elsev) =>
        eval(cond) match {
          ...
          case Val.False =>
            eval(elsev)
          case cond =>
            emit.select(materialize(cond),
                        materialize(eval(thenv)),
                        materialize(eval(elsev)),
                        unwind)
        }
      ...
      case Op.As(ty, obj) =>
        eval(obj) match {
          ...
          case obj =>
            obj.ty match {
              case ExactClassRef(cls) =>
                obj
              case objty =>
                glb(ty, obj.ty).fold {
                  log(
                    s"asinstanceof: no glb between ${obj.show} and ${ty.show}")
                  emit.as(ty, materialize(obj), unwind)
                } { glbty =>
                  if (glbty == obj.ty) {
                    obj
                  } else {
                    emit.as(glbty, materialize(obj), unwind)
                  }
                }
            }
        }
