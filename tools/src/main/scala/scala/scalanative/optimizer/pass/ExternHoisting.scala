...
class ExternHoisting(implicit top: Top) extends Pass {
  ...
  override def onDefns(defns: Seq[Defn]): Seq[Defn] = {
    ...
    defns.foreach {
      case defn @ Defn.Declare(attrs, name, _)
          if attrs.isExtern && name.id.startsWith("extern.") =>
        buf += super.onDefn(defn.copy(name = stripName(name)))
      case defn @ Defn.Define(attrs, name, _, _)
          if attrs.isExtern && name.id.startsWith("extern.") =>
        buf += super.onDefn(defn.copy(name = stripName(name)))
      case defn @ Defn.Const(attrs, name, _, _)
          if attrs.isExtern && name.id.startsWith("extern.") =>
        buf += super.onDefn(defn.copy(name = stripName(name)))
      case defn @ Defn.Var(attrs, name, _, _)
          if attrs.isExtern && name.id.startsWith("extern.") =>
        buf += super.onDefn(defn.copy(name = stripName(name)))
      case defn =>
  ...
  override def onVal(value: Val) = value match {
    case Val.Global(name @ Global.Member(_, id), ty)
        if id.startsWith("extern.__") =>
      Val.Global(stripName(name), ty)
    case Val.Global(name @ Ref(node), ty)
        if node.attrs.isExtern && name.id.startsWith("extern.") =>
      Val.Global(stripName(name), ty)
    case _ =>
      super.onVal(value)
  }
