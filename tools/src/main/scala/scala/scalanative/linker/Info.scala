...
final class Info(val deps: Seq[Dep],
                 val links: Seq[Attr.Link],
                 val sigs: Seq[String],
                 val defn: Defn)

object Info {
  def apply(defn: Defn): Info = {
    val deps = mutable.UnrolledBuffer.empty[Dep]
    val sigs = mutable.UnrolledBuffer.empty[String]
    ...
    onDefn(defn)
    new Info(deps, defn.attrs.links, sigs, defn)
  }
}
