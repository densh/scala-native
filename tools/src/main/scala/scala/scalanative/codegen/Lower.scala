...
object Lower {
  ...
  private final class Impl(implicit meta: Metadata) extends Transform {
    ...
    def genBoxOp(buf: Buffer, n: Local, op: Op.Box, unwind: Next): Unit = {
      ...
      val boxTy =
        Type.Function(Seq(Type.Module(moduleName), Type.unbox(ty)), ty)
    ...
    def genUnboxOp(buf: Buffer, n: Local, op: Op.Unbox, unwind: Next): Unit = {
      ...
      val unboxTy =
        Type.Function(Seq(Type.Module(moduleName), ty), Type.unbox(ty))
  ...
  val BoxTo: Map[Type, Global] = Seq(
    ...
    val boxty = Type.Class(Global.Top(name))
    val module =
  ...
  val unitTy =
    Type.StructValue(unitName.member(Sig.Generated("layout")), Seq(Type.Ptr))
  val unitConst =
  ...
  val arrayAllocSig = Type.typeToArray.map {
    case (ty, arrname) =>
      val Global.Top(id) = arrname
      ty -> Type.Function(Seq(Type.Module(Global.Top(id + "$")), Type.Int),
                          Type.Class(arrname))
  }.toMap
  ...
  val arraySnapshotSig = Type.typeToArray.map {
    case (ty, arrname) =>
      val Global.Top(id) = arrname
      ty -> Type.Function(
        Seq(Type.Module(Global.Top(id + "$")), Type.Int, Type.Ptr),
        Type.Class(arrname))
  }.toMap
  ...
  val arrayApplySig = Type.typeToArray.map {
    case (ty, arrname) =>
      ty -> Type.Function(Seq(Type.Class(arrname), Type.Int), ty)
  }.toMap
  ...
  val arrayUpdate = Type.typeToArray.map {
    case (ty @ Type.Unit, arrname) =>
      ty -> Global.Member(
        ...
        Sig.Method("update",
                   Seq(Type.Int,
                       Type.Class(Global.Top("scala.runtime.BoxedUnit")),
                       Type.Unit)))
    case (ty, arrname) =>
      ty -> Global.Member(arrname,
  ...
  val arrayUpdateSig = Type.typeToArray.map {
    case (ty, arrname) =>
      ty -> Type.Function(Seq(Type.Class(arrname), Type.Int, ty), Type.Unit)
  }.toMap
  val arrayLength =
