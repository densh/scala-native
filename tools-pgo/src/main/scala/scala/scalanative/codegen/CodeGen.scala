...
object CodeGen {
  ...
  def apply(config: tools.Config, assembly: Seq[Defn]): Unit =
    Scope { implicit in =>
          ...
          case (k, defns) =>
            ...
            val impl =
              new Impl(config.target,
                       ...
                       sorted,
                       workdir,
                       config.profileDirectedBlockPlacement)
            val outpath = k + ".ll"
            val buffer  = impl.gen()
            buffer.flip
      ...
      def release(): Unit = {
        ...
        val buffer = impl.gen()
        buffer.flip
        workdir.write(Paths.get("out.warm.ll"), buffer)
      }

      def releaseSplitFrozen(): Unit = {
        val hot  = assembly.filter(_.attrs.inline != Attr.NoInline)
        val cold = assembly.filter(_.attrs.inline == Attr.NoInline)
        locally {
          val sorted = hot.sortBy(_.name.show)
          val impl = new Impl(config.target,
                              env,
                              sorted,
                              workdir,
        ...
        locally {
          val sorted = cold.sortBy(_.name.show)
          val impl = new Impl(config.target,
                              env,
                              sorted,
                              workdir,
          ...
          val buffer = impl.gen()
          buffer.flip
          workdir.write(Paths.get("out.cold.ll"), buffer)
        }
      }

      config.mode match {
        case tools.Mode.Debug =>
          debug()
        case tools.Mode.Release if config.pruneUntakenBranches =>
          releaseSplitFrozen()
        case tools.Mode.Release =>
          release()
      }
    }
  ...
  private final class Impl(target: String,
    ...
    def genBlock(block: Block)(implicit cfg: CFG, fresh: Fresh): Unit = {
      val Block(name, params, insts, _, isEntry) = block
    ...
    def genInst(inst: Inst)(implicit cfg: CFG, fresh: Fresh): Unit =
      inst match {
        case inst: Inst.Let =>
          genLet(inst)
    ...
    def genBranchWeights(nexts: Seq[Next])(implicit cfg: CFG): Unit = {
      if (profileDirectedBlockPlacement) {
        val kind = "!\"branch_weights\""
        val values = nexts.map { next =>
          "i32 " + cfg.find(next.name).warmth
        }
        str(", !prof !")
